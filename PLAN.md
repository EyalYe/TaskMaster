...
6. **Cohesion pass. ✅ Done.** Hint-bar **action glyphs** wired for the conventional tokens
   (`DON`/`OK`→check, `OPN`/`SEL`→select, `MNU`→menu, `RST`→reset; text stays the fallback), so the
   Launcher / Settings / task apps read consistently. Snow weather glyph added. **City "not found"**
   surfaces in **Settings → Device info** (`Weather: '<city>' not found`) instead of silently showing no
   weather. Dead code removed (`ui/hint_bar.c`, the old raw-sh1106 renderer). Docs refreshed.
