...
void ui_frame_init(void)
{
    ...
    /* Home (OS-fixed). */
    lv_obj_t *home = make_box(s_bar, HINT_BAR_X, 1, HINT_BAR_W, 16);
    lv_obj_align(make_label(home, "HOM"), LV_ALIGN_CENTER, 1, 0);
    ...
    lv_obj_t *enc = make_box(s_bar, HINT_BAR_X, 18, HINT_BAR_W, 28);
    s_rotate_lbl = make_label(enc, "<>");
    lv_obj_align(s_rotate_lbl, LV_ALIGN_TOP_MID, 1, 2);
    lv_obj_t *divln = lv_obj_create(enc);
    lv_obj_remove_style_all(divln);
    lv_obj_set_size(divln, HINT_BAR_W - 2, 1);
    lv_obj_align(divln, LV_ALIGN_TOP_MID, 0, 12);
    lv_obj_set_style_bg_color(divln, lv_color_white(), 0);
    lv_obj_set_style_bg_opa(divln, LV_OPA_COVER, 0);
    s_click_lbl = make_label(enc, "");
    lv_obj_align(s_click_lbl, LV_ALIGN_BOTTOM_MID, 1, -2);

    /* Select. */
    lv_obj_t *sel = make_box(s_bar, HINT_BAR_X, 47, HINT_BAR_W, 16);
    s_select_lbl = make_label(sel, "");
    lv_obj_align(s_select_lbl, LV_ALIGN_CENTER, 1, 0);
